...
// Copyright (c) 2018 The Book Core developers
...
const char * const BITCOIN_CONF_FILENAME = "book.conf";
const char * const BITCOIN_PID_FILENAME = "bookd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "book" is a composite category enabling all Book-related debug output
            if(ptrCategory->count(string("book"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BookCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BookCore
    // Mac: ~/Library/Application Support/BookCore
    // Unix: ~/.bookcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BookCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BookCore";
#else
    // Unix
    return pathRet / ".bookcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty book.conf if it does not excist
        ...
        if (configFile != NULL) {
        	...
        	LogPrintf("book.conf file not found or can't be created\n");
    ...
    {
        // Don't overwrite existing settings so command line settings override book.conf
...
void AddSeedsToConfigFile(FILE* configFile) {
        fprintf(configFile,"onlynet=ipv41\r\n");
	fprintf(configFile,"addnode=206.189.26.208:31241\r\n");
	fprintf(configFile,"addnode=178.74.193.129\r\n");
	fprintf(configFile,"addnode=77.66.200.240\r\n");
	fprintf(configFile,"addnode=173.249.59.229\r\n");
